<script setup>
import { listUser, createStudent, listStudent } from '@/api/account';
...
<template>
    ...
    <Dialog v-model:visible="visible" maximizable modal header="Create Student" :style="{ width: '50vw' }">
        <div class="flex flex-column">
            ...
            <Dropdown v-model="selectedUser" :options="listNewUser" optionLabel="name" placeholder="Select a Country" class="w-full md:w-40rem mb-5">
    ...
    <Dialog v-model:visible="showStudentDetail" maximizable modal header="Detail Student" :style="{ width: '80vw' }">
        <div class="card text-6xl text-blue-800 font-bold flex justify-content-center" v-if="!detailsStudent.length">No Data</div>
        <div v-else><Chart type="bar" :data="chartData" :options="chartOptions" class="h-30rem" /></div>
    </Dialog>
    <div class="card mt-4">
        <DataTable :value="Arrstudents" dataKey="id" responsiveLayout="scroll">
            ...
            <Column field="action" header="Action" :showFilterMatchModes="false" style="min-width: 13rem" class="">
                <template #body="{ data }">
                    <div>
                        <Button class="mr-1" label="Detail" outlined @click="detailStudent(data.id)" />
                    </div>
                </template>
            </Column>
        </DataTable>
